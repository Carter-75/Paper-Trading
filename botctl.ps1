...
function Write-BotStartScript([string]$BotCommand) {
  ...
  $content += '  Add-Content -Path .\\bot.log -Value ("MUTEX_ERROR: " + $err + " " + (Get-Date).ToString("s"))'
  $content += '  Write-Host ("MUTEX_ERROR: " + $err) -ForegroundColor Red'
  $content += '}'
  $content += ''
