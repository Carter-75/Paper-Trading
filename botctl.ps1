...
function New-StartScript([string]$line) {
    ...
    Add-Content -Path $ScriptPS1 -Value '        try {'
    Add-Content -Path $ScriptPS1 -Value '          if (-not (Test-Path $Path)) { Start-Sleep -Seconds 5; continue }'
    Add-Content -Path $ScriptPS1 -Value '          $lines = Get-Content $Path -ErrorAction SilentlyContinue'
    Add-Content -Path $ScriptPS1 -Value '          if ($null -eq $lines) { Start-Sleep -Seconds 5; continue }'
    Add-Content -Path $ScriptPS1 -Value '          if ($lines.Count -le $Max) { Start-Sleep -Seconds 5; continue }'
    Add-Content -Path $ScriptPS1 -Value '          $initLine = $lines | Where-Object { $_ -match ''^INIT '' } | Select-Object -First 1'
    Add-Content -Path $ScriptPS1 -Value '          $kept = $lines | Select-Object -Last $Max'
    Add-Content -Path $ScriptPS1 -Value '          if ($initLine -and $kept -notcontains $initLine) {'
    Add-Content -Path $ScriptPS1 -Value '            $kept = @($initLine) + ($kept | Select-Object -Skip 1)'
    Add-Content -Path $ScriptPS1 -Value '          }'
    Add-Content -Path $ScriptPS1 -Value '          Set-Content -Path $Path -Value $kept -ErrorAction SilentlyContinue'
    Add-Content -Path $ScriptPS1 -Value '        } catch { }'
    Add-Content -Path $ScriptPS1 -Value '        Start-Sleep -Seconds 5'
    Add-Content -Path $ScriptPS1 -Value '      }'
    Add-Content -Path $ScriptPS1 -Value '    } -ArgumentList $LogPath, $MaxLines'
    Add-Content -Path $ScriptPS1 -Value '  } catch { return $null }'
    Add-Content -Path $ScriptPS1 -Value '}'
    Add-Content -Path $ScriptPS1 -Value ''
    Add-Content -Path $ScriptPS1 -Value 'function Stop-LogTrimJob($job) {'
    Add-Content -Path $ScriptPS1 -Value '  if ($null -eq $job) { return }'
    Add-Content -Path $ScriptPS1 -Value '  try { Stop-Job -Job $job -Force -ErrorAction SilentlyContinue | Out-Null } catch { }'
    Add-Content -Path $ScriptPS1 -Value '  try { Remove-Job -Job $job -Force -ErrorAction SilentlyContinue | Out-Null } catch { }'
    Add-Content -Path $ScriptPS1 -Value '}'
    Add-Content -Path $ScriptPS1 -Value ''
